...
const User = ({ item }) => {
    ...
    useEffect(()=>{
        const fetchFriendRequests = async()=>{
            try{
                const response = await fetch(`http://192.168.0.104:4000/friend-requests/sent/${userId}`);
    ...
    useEffect(()=>{
        const fetchUserFriends = async() =>{
            try{
                const response = await fetch(`http://192.168.0.104:4000/friends/${userId}`);
    ...
    const sendFriendRequest = async(currentUserId, selectedUserId) =>{
        try{
            const response = await fetch("http://192.168.0.104:4000/friend-request", {
                method: "POST",
                headers: {
    ...
    return (
        <Pressable style={{flexDirection: "row", alignItems: "center", marginVertical: 10}}>
            ...
            {userFriends.includes(item._id) ? (
                ...
                <Pressable onPress={()=>sendFriendRequest(userId, item._id)} style={{
